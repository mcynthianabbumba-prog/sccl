...
export const DoctorLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Fetch role directly — auth context hasn't updated yet
    const { data: prof } = await supabase
      .from('profiles').select('role').eq('id', data.user.id).single();

    const role = prof?.role;
    if      (role === 'super_admin')    navigate('/admin/owner',      { replace: true });
    else if (role === 'hospital_admin') navigate('/admin/hospital',   { replace: true });
    else if (role === 'doctor')         navigate('/doctor/dashboard', { replace: true });
    else {
      await supabase.auth.signOut();
      setError('Your account does not have staff access. Use the main login page for patient access.');
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      {[{s:400,t:-100,r:-100,o:.04},{s:300,b:-80,l:-80,o:.06},{s:180,top:'40%',left:'20%',o:.03}].map((c,i)=>(
        <div key={i} style={{ position:'absolute',width:c.s,height:c.s,borderRadius:'50%',background:'#fff',opacity:c.o,top:c.t,bottom:c.b,left:c.l,right:c.r,pointerEvents:'none' }}/>
      ))}
        ...
        <div style={{ textAlign:'center', marginBottom:32 }}>
          <Link to="/" style={{ display:'inline-flex',alignItems:'center',gap:10,marginBottom:28,textDecoration:'none' }}>
            <div style={{ width:40,height:40,background:'rgba(255,255,255,.15)',borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',border:'1px solid rgba(255,255,255,.2)' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="4" fill="white"/><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4" stroke="white" strokeWidth="2" strokeLinecap="round"/></svg>
            </div>
